...
import 'package:surf_practice_chat_flutter/features/chat/presentation/chat_screen_scope.dart';
import 'package:surf_practice_chat_flutter/features/core/presentation/l10n/app_localizations.dart';
import 'package:surf_practice_chat_flutter/features/core/presentation/util/color_utils.dart';
import 'package:surf_practice_chat_flutter/features/core/presentation/widgets/app_icon_button.dart';
import 'package:surf_practice_chat_flutter/features/core/presentation/widgets/keyboard_height_listener.dart';
            ...
            delegate: FlutterListViewDelegate(
              (context, index) {
                ...
                final previousMessage =
                    index < widget.messages.length - 1 ? widget.messages.elementAt(index + 1) : null;
...
class _ScrollToBottomButton extends StatefulWidget {
  ...
  @override
  _ScrollToBottomButtonState createState() => _ScrollToBottomButtonState();
}

class _ScrollToBottomButtonState extends State<_ScrollToBottomButton> with SingleTickerProviderStateMixin {
  late final AnimationController _animationController;
  late final VoidCallback _scrollListener;
  bool _isVisible = false;

  FlutterListViewController get _controller => widget.controller;
  ...
  void initState() {
    ...
    _scrollListener = () {
      if (_controller.hasClients) {
        ...
        if (distanceToBottom > _controller.position.viewportDimension * 2) {
          if (_isVisible == false) {
            _setVisibilityTo(true);
          }
        } else {
          if (_isVisible == true) {
            _setVisibilityTo(false);
          }
        }
      }
    };
  ...
  void _setVisibilityTo(bool value) {
    setState(() {
      _isVisible = value;
    });
            ...
            Row(
              children: <Widget>[
                BlocBuilder<CurrentMessageBloc, CurrentMessageState>(
                  buildWhen: (prev, curr) =>
                      prev.coordinates != curr.coordinates && (prev.coordinates == null) != (curr.coordinates == null),
                  builder: (context, state) {
                    return _AttachGeoPointIcon(isActive: state.coordinates != null);
                  },
                ),
...
class _AttachGeoPointIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AppIconButton(
      onPressed: () => ChatScreenScope.attachGeoPoint(context),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          if (nextIsTheSameUser) const SizedBox(width: 8.0),
          Expanded(
            child: Bubble(
              nipWidth: 8.0,
              nip: nextIsTheSameUser
                  ? null
                  : isMessageSentByThisUser
